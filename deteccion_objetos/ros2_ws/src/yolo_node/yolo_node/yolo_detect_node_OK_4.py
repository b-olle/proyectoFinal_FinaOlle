...
class YOLONode(Node):
    ...
    def callback(self, msg):
        ...
        '''# Solo analizamos si HAY detección
        if len(detections) > 0:
        	# Tomamos la clase del objeto principal (mayor confianza)
        	top_class_id = int(detections[0].cls.item())
        	class_name = results[0].names[top_class_id]
        	...
        	if len(self.detection_history) == self.CONFIRMATION_LIMIT:
        		# Contamos cuál es el objeto más frecuente en las últimas 10
        		most_common, count = Counter(self.detection_history).most_common(1)[0]
        
        		# Si aparece al menos en 8 de las últimas 10 (80% de estabilidad)
        		if count >= self.CONFIRMATION_LIMIT*0.8:
        			nombre_final = self.traductor.get(most_common, most_common)
        			# self.get_logger().info(f"Veo: {most_common}")
        			self.get_logger().info(f"Veo {nombre_final}")
        			self.detection_history.clear()
        			# Accionar aquí...
        			
        		elif count >= self.CONFIRMATION_LIMIT*0.3:
        			self.get_logger().info(f"Parece que veo algo")'''
        '''else:
