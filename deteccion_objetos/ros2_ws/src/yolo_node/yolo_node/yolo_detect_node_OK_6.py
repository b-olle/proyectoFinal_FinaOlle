...
'''import rclpy
from rclpy.node import Node
from sensor_msgs.msg import Image, LaserScan # Agregado LaserScan
from cv_bridge import CvBridge
import cv2
from ultralytics import YOLO
from sensor_msgs.msg import CompressedImage
...
class YOLONode(Node):
    ...
    def __init__(self):
        ...
        # --- NUEVO: PARÁMETROS LIDAR Y CÁMARA ---
        self.latest_scan = None
        self.CAMERA_FOV_HORIZ = 62.2  # <--- AJUSTAR SEGÚN TU CÁMARA (Grados)
        # Entiendo que está ok para la rpicam
        # self.LIDAR_OFFSET_ANGLE = 0.0 # Ajuste de rotación si es necesario
        self.LIDAR_OFFSET_ANGLE = math.radians(0.0)
    ...
    def get_distance_to_bbox(self, bbox, img_width):
        """
        Calcula distancia promedio manejando el cruce por cero (0 grados) del Lidar.
        """
        if self.latest_scan is None:
        ...
        # 2. Calcular ángulos de los bordes del objeto (Grados relativos a la cámara)
        # x_min (izquierda objeto) -> Ángulo mayor (más a la izquierda/positivo)
        # x_max (derecha objeto) -> Ángulo menor (más a la derecha/negativo)
        angle_left_deg = (center_pixel - x_min) / pixels_per_degree 
        angle_right_deg = (center_pixel - x_max) / pixels_per_degree

        # 3. Función auxiliar para obtener índice en el array del Lidar
        def get_index(angle_deg):
            # Convertir a radianes y ajustar offset
            rad = math.radians(angle_deg) + self.LIDAR_OFFSET_ANGLE
            
            # Normalizar a [0, 2*PI] para que -30 grados sea 330 grados
            if rad < 0: rad += 2 * math.pi
            elif rad >= 2 * math.pi: rad -= 2 * math.pi
            ...
            # Asegurar que esté dentro del array
            return max(0, min(len(msg.ranges) - 1, idx))

        # 4. Obtenemos índices de inicio y fin
        # IMPORTANTE: El Lidar barre en sentido antihorario (CCW).
        # Queremos ir desde el borde derecho (ángulo menor) hasta el izquierdo (mayor).
        idx_start = get_index(angle_right_deg) # Ej: -10° -> idx 350
        idx_end = get_index(angle_left_deg)    # Ej: +10° -> idx 10
        
        measurements = []

        # 5. Extracción inteligente (Manejo del cruce por cero)
        if idx_start > idx_end:
            # CASO CRUCE: El objeto está justo al frente (cruza el grado 0)
            # Tomamos desde idx_start hasta el final... Y desde 0 hasta idx_end
            measurements = msg.ranges[idx_start:] + msg.ranges[:idx_end + 1]
        else:
            # CASO NORMAL: El objeto está totalmente a un lado
            measurements = msg.ranges[idx_start : idx_end + 1]
        ...
        if valid_ranges:
            # Ordenamos de menor a mayor (más cerca a más lejos)
            valid_ranges.sort()
            
            # Nos quedamos con el 40% de los puntos más cercanos
            # (Asumimos que al menos el 40% del ancho del bounding box es realmente el objeto)
            n_samples = max(1, int(len(valid_ranges) * 0.4))
            closest_ranges = valid_ranges[:n_samples]
    ...
    def callback(self, msg):
        ...
        if len(detections) > 0:
            ...
            # 3. Analizamos
            most_common, count = Counter(self.detection_history).most_common(1)[0]
            nombre_es = self.traductor.get(most_common, most_common) # Entiendo que siempre se mostraría el más común de los contados
            hist_len = len(self.detection_history)
            ...
            if hist_len == self.CONFIRMATION_LIMIT and count >= 8:
                ...
                bbox = top_det.xyxy.cpu().numpy().astype(int).squeeze()
                img_w = frame.shape[1]
                dist = self.get_distance_to_bbox(bbox, img_w)
                dist_str = f"a {dist:.2f}m" if dist else "(distancia desc.)"
                # -----------------------------
