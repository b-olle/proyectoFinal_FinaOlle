...
class YOLONode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('lidar_offset', 8.0)
        ...
        #self.LIDAR_OFFSET_ANGLE = math.radians(8.0)
        ...
        # Publicación de datos de objetos 
        self.target_pub = self.create_publisher(String, '/object_detection_data', 10)

        self.get_logger().info(f'YOLO Node iniciado con modelo: {self.model_path}')
        
        self.is_rotating = False
        self.ROTATION_THRESHOLD = 0.05  # rad/s (ajústar según robot)
        
        self.odom_sub = self.create_subscription(
            Odometry,
            '/odom', # tópico de velocidad
            self.odom_callback,
            qos_profile_sensor_data
        )

    def odom_callback(self, msg):
        # msg.twist.twist.angular.z nos da la velocidad de rotación en Z
        angular_z = abs(msg.twist.twist.angular.z)
        self.is_rotating = angular_z > self.ROTATION_THRESHOLD
    ...
    def get_distance_to_bbox(self, bbox, img_width, img_timestamp):
        ...
        # AGREGADO
        # Leemos el parámetro de RQT y lo convertimos a radianes
        offset_val = self.get_parameter('lidar_offset').get_parameter_value().double_value
        self.LIDAR_OFFSET_ANGLE = math.radians(offset_val)
    ...
    def callback(self, msg):
        ...
        if len(detections) > 0:
            ...
            if hist_len == self.CONFIRMATION_LIMIT and count >= 8:
                ...
                # --- NUEVO: CALCULO DE ÁNGULO Y PUBLICACIÓN ---
                if dist is not None:
                
                    # ---> FILTRO <---
                    if self.is_rotating:
                        self.get_logger().info("Robot girando. Ignorando publicación para markers.")
                        self.detection_history.clear()
                        return  # Corta la ejecución acá para no publicar el JSON
                    
                    # 1. Calcular centro del Bounding Box en X
                    x_center = (bbox[0] + bbox[2]) / 2.0
                    
                    # 2. Calcular ángulo (grados) respecto al centro de la cámara (asumiendo alineado con Lidar 0)
                    # Nota: (Centro - objeto) / pixeles_por_grado
                    pixels_per_degree = img_w / self.CAMERA_FOV_HORIZ
                    angle_deg = (img_w / 2.0 - x_center) / pixels_per_degree
                    
                    # 3. Crear mensaje JSON
                    data = {
                        "class": nombre_es,       # Ej: "una silla"
                        "class_id": class_name,   # Ej: "chair" (nombre original en inglés)
                    ...
                    self.target_pub.publish(msg_str)
        ...
        for det in detections:
            ...
            cv2.putText(frame, f'{nombre_clase} {conf:.2f}', (xyxy[0], xyxy[1]-5), cv2.FONT_HERSHEY_SIMPLEX, 0.85, (0, 255, 255), 2)
